...
#pragma omp parallel for
    for (j = 0; j < RK4IMP_STAGE; j++)
      {
        int s =
          GSL_ODEIV_FN_EVU (sys, t + c[j] * h, &YZ[j * dim], &fYZ[j * dim]);
...
#pragma omp parallel for
    for (j = 0; j < RK4IMP_STAGE; j++)
      {
        int s = GSL_ODEIV_FN_EVU (sys, t + c[j] * h / 2.0, &YZ[j * dim],
                                   &fYZ[j * dim]);
        if (s != GSL_SUCCESS)
          {
            return s;
...
#pragma omp parallel for
    for (j = 0; j < RK4IMP_STAGE; j++)
      {
        int s =
          GSL_ODEIV_FN_EVU (sys, t + h / 2.0 + c[j] * h / 2.0, &YZ[j * dim],
                             &fYZ[j * dim]);
        if (s != GSL_SUCCESS)
          {
            return s;
  ...
  if (dydt_out != NULL)
    {
      int s = GSL_ODEIV_FN_EVU (sys, t + h, y, dydt_out);
